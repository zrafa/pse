...
computados sean correctos: tambien es necesario que sean producidos dentro
...
Para cada una de estas se definen tambien, con precision, los tiempos de respuesta maximos permitido por parte del sistema (sobre todo para las funcionalidades
activadas por eventos). Luego, este diseño se implementa\footnote{Etapa de desarrollo y programación del código fuente} generalmente como tareas independientes, que seran ejecutadas de manera concurrente en tiempo de ejecucion. 
...
\section{XINU en atmega328p RTOS}
...
El codigo fuente del kernel de XINU contiene aproximadamente 
2500 lineas de codigo en lenguaje C, lo que lo hace completamente 
...
como RTOS, sino tambien entender, modificar y extender su 
...
\begin{itemize}
	\item Versión RTOS (codigo fuente):  \textit{https://github.com/zrafa/xinu-avr-rtos}
...
Durante ese tiempo, el planificador cede la CPU a otros procesos listos. 
Cuando el tiempo vence, el proceso pasa al estado LISTO y será ejecutado 
...
50ms en este caso, en cuanto se active por el RTOS luego de 50ms,
...
un tipico sistema embebido desarrollado utilizlando un RTOS.
...
Cada proceso utiliza la CPU durante un milisegundo (esto es configurable 
...
le asigna la CPU al proceso \texttt{null}.

En total, existen aproximadamente 1200 bytes de memoria disponible
para las tareas y aplicación embebida.
\texttt{system/initialize.c} crea el proceso main, y le asigna 256 bytes de esa memoria libre.
